...
class Net(nn.Module):
    ...
    def __init__(self, layer_sizes):
        ...
        for l in range(len(layer_sizes)-1):
            w = nn.Linear(layer_sizes[l],layer_sizes[l+1], bias = False).double()
            w.weight.data.copy_(torch.abs(w.weight.data))
    ...
    def forward(self, Xs):
        ...
        factors = [[Xs[k]] for k in range(len(Xs))]
        
        
        for l in range(self.depth):
            for k in range(len(Xs)):         
                factors[k].append(self.weights[l](factors[k][-1]))
...
class Energy_Loss_Tensor(nn.Module):
    ...
    def forward(self, net, X, factors):
        ...
        for d in range(depth+1):
            ...
            approx.append(outer_product([factors[i][d] for i in range(k)]))
